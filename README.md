...
A controller will have all the business logic related to one object or one api path ( `/v1/vehicles` )

Naming convention: If your api is `api/v1/vehicles` then name your controller as `vehicle.controller.js`
`vehicle.controller.js` will have all the business logic that will be performed at `api/v1/vehicles/...` endpoints.
...
To create a api/v1/vehicles/... API routes:

1. Create `vehicle.controller.js` in `controllers`
2. Create `vehicle.route.js` in `routes`
3. Create `vehicle.validation.js` in `validations`
4. Create `vehicle.collection.js` in `repository/mongo`
5. You might want to create `vehicle.service.js` inside `services`
...
<br>
`vehicle.route.js` should look like:

```
const validate = require('express-validation');

const controller = require('../../controllers/vehicle.controller');
const validation = require('../../validations/vehicle.validation');


...
router.route('/')
    .get(validate(validation.getVehicles), controller.getVehicles)
    .post(validate(validation.createVehicle), controller.createVehicle)
```

<br>
`vehicle.controller.js` should make use of functions exported by `vehicle.collection.js`
